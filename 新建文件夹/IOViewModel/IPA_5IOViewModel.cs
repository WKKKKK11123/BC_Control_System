...
namespace BC_Control_System.ViewModel.Status.IOViewModel
{
    ...
    public class IPA_5IOViewModel : BindableBase, INavigationAware
    {
        private List<DataClass> list;
        private PropertyInfo[] propertyInfos;
        public IPA_5IOClass TankStatusClass { get; set; }
        public double Tank5OutsideLevel { get; set; }
        public double Tank5InsideLevel { get; set; }
        public bool IsAuto { get; set; } = true;
        public IPA_5IOViewModel()
        { 
            TankStatusClass= new IPA_5IOClass();
        ...
        private void UpdateValue()
        {
            while (true)
            {
                try
                {
                    ...
                    var targetItem = list.FirstOrDefault(x => x.ParameterName == "Tank5OutsideLevel");
                    if (targetItem != null && double.TryParse(targetItem.Value, out double parsedValue))
                    {
                        Tank5OutsideLevel = parsedValue != 0 ? 5 - parsedValue : parsedValue;
                    }

                    var targetItem1 = list.FirstOrDefault(x => x.ParameterName == "Tank5InsideLevel");
