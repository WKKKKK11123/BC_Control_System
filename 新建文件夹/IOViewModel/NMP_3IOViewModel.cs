...
namespace BC_Control_System.ViewModel.Status.IOViewModel
{
    ...
    public class NMP_3IOViewModel : BindableBase, INavigationAware
    {
        private List<DataClass> list;
        private PropertyInfo[] propertyInfos;
        public NMP_3IOClass TankStatusClass { get; set; }
        public double Tank3OutsideLevel { get; set; }
        public double Tank3InsideLevel { get; set; }
        public double Tank3CoolingLevel { get; set; }
        public bool IsAuto { get; set; } = true;
        public NMP_3IOViewModel()
        { 
            TankStatusClass = new NMP_3IOClass();
        ...
        private readonly Dictionary<string, string> _buttonTips = new Dictionary<string, string>
{
    ...
    { "Heating", "确认打开加热！" }
        ...
        private void WritePLC(string parameterName)
        {
            ...
            // 写入相反值
            PLC4CommonMethods.CommonWrite(address, (!current).ToString().ToLower());
        }
        ...
        private void UpdateValue()
        {
            while (true)
            {
                try
                {
                    ...
                    var targetItem = list.FirstOrDefault(x => x.ParameterName == "Tank3OutsideLevel");
                    if (targetItem != null && double.TryParse(targetItem.Value, out double parsedValue))
                    {
                        Tank3OutsideLevel = parsedValue != 0 ? 5 - parsedValue : parsedValue;
                    }

                    var targetItem1 = list.FirstOrDefault(x => x.ParameterName == "Tank3InsideLevel");
                    if (targetItem1 != null && double.TryParse(targetItem1.Value, out double parsedValue1))
                    {
                        ...
                        {
                            Tank3InsideLevel = parsedValue1;
                        }


                    }

                    var targetItem2 = list.FirstOrDefault(x => x.ParameterName == "Tank3CoolingLevel");
                    if (targetItem2 != null && double.TryParse(targetItem2.Value, out double parsedValue2))
                    {
                        Tank3CoolingLevel = parsedValue2 != 0 ? 5 - parsedValue2 : parsedValue2;
                    }
                    Thread.Sleep(300);
                }
