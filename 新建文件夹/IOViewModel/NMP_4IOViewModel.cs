...
namespace BC_Control_System.ViewModel.Status.IOViewModel
{
    ...
    public class NMP_4IOViewModel : BindableBase, INavigationAware
    {
        private List<DataClass> list;
        private PropertyInfo[] propertyInfos;
        public NMP_4IOClass TankStatusClass { get; set; }
        public double Tank4OutsideLevel { get; set; }
        public double Tank4InsideLevel { get; set; }
        public double Tank4CoolingLevel { get; set; }
        public bool IsAuto { get; set; } = true;
        public NMP_4IOViewModel()
        {
            TankStatusClass = new NMP_4IOClass();
        ...
        private void UpdateValue()
        {
            while (true)
            {
                try
                {
                    ...
                    var targetItem = list.FirstOrDefault(x => x.ParameterName == "Tank4OutsideLevel");
                    if (targetItem != null && double.TryParse(targetItem.Value, out double parsedValue))
                    {
                        Tank4OutsideLevel = parsedValue != 0 ? 5 - parsedValue : parsedValue;
                    }

                    var targetItem1 = list.FirstOrDefault(x => x.ParameterName == "Tank4InsideLevel");
                    if (targetItem1 != null && double.TryParse(targetItem1.Value, out double parsedValue1))
                    {
                        ...
                        {
                            Tank4InsideLevel = parsedValue1;
                        }

                    }

                    var targetItem2 = list.FirstOrDefault(x => x.ParameterName == "Tank4CoolingLevel");
                    if (targetItem2 != null && double.TryParse(targetItem2.Value, out double parsedValue2))
                    {
                        Tank4CoolingLevel = parsedValue2 != 0 ? 5 - parsedValue2 : parsedValue2;
                    }
                    Thread.Sleep(300);
                }
                catch (Exception ee)
