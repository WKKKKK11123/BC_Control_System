...
namespace BC_Control_System.ViewModel.Status.IOViewModel
{
    ...
    public class SYS9070_1IOViewModel : BindableBase, INavigationAware
    {

        
        private List<DataClass> list;
        private PropertyInfo[] propertyInfos;
        public SYS9070_1IOClass TankStatusClass { get; set; }
        public double Tank1OutsideLevel { get; set; }
        public double Tank1InsideLevel { get; set; }
        public double Tank1CoolingLevel { get; set; }
        public bool IsAuto { get; set; } = true;
        public SYS9070_1IOViewModel(IDialogService dialogService)
        {
            TankStatusClass = new SYS9070_1IOClass();
        ...
        private readonly Dictionary<string, string> _buttonTips = new Dictionary<string, string>
{
    ...
    { "Heating", "确认打开加热！" }
        ...
        private void WritePLC(string parameterName)
        {
            ...
            if (item == null || !PLC5CommonMethods.Device.IsConnected)
                return;

            string address = item.ValueAddress;
            bool current = PLC5CommonMethods.Device[address] is bool b && b;
            ...
            // 写入相反值
            PLC5CommonMethods.CommonWrite(address, (!current).ToString().ToLower());
        }
        ...
        private void UpdateValue()
        {
            while (true)
            {
                try
                {
                    if (PLC5CommonMethods.Device.IsConnected)
                    {
                    ...
                    var targetItem = list.FirstOrDefault(x => x.ParameterName == "Tank1OutsideLevel");
                    if (targetItem != null && double.TryParse(targetItem.Value, out double parsedValue))
                    {
                        Tank1OutsideLevel = parsedValue != 0 ? 5 - parsedValue : parsedValue;

                        //if (parsedValue != 0)
                        //{
                        //    ActualLevel = 5 - parsedValue;
                        //}
                        //else
                        //{ 
                        //    ActualLevel = parsedValue; 
                        //}
                    }

                    var targetItem1 = list.FirstOrDefault(x => x.ParameterName == "Tank1InsideLevel");
                    if (targetItem1 != null && double.TryParse(targetItem1.Value, out double parsedValue1))
                    {
                        ...
                        {
                            Tank1InsideLevel = parsedValue1;
                        }
                    }

                    var targetItem2 = list.FirstOrDefault(x => x.ParameterName == "Tank1CoolingLevel");
                    if (targetItem2 != null && double.TryParse(targetItem2.Value, out double parsedValue2))
                    {
                        Tank1CoolingLevel = parsedValue2 != 0 ? 5 - parsedValue2 : parsedValue2;
                    }
        ...
        private bool GetBoolValueByParameterName(string parameterName)
        {
            ...
            if (item != null && PLC5CommonMethods.Device.IsConnected)
            { 
                if (PLC5CommonMethods.Device[item.ValueAddress] is bool b)
                    return b;
            }
            return false;
        }
