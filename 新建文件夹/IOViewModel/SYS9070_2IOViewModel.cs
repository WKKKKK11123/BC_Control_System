...
namespace BC_Control_System.ViewModel.Status.IOViewModel
{
    ...
    public class SYS9070_2IOViewModel : BindableBase, INavigationAware
    {
        private List<DataClass> list;
        private PropertyInfo[] propertyInfos;
        public SYS9070_2IOClass TankStatusClass { get; set; }
        public double Tank2OutsideLevel { get; set; }
        public double Tank2InsideLevel { get; set; }
        public double Tank2CoolingLevel { get; set; }
        public bool IsAuto { get; set; } = true;
        public SYS9070_2IOViewModel()
        {
            TankStatusClass=new SYS9070_2IOClass();
        ...
        private void UpdateValue()
        {
            while (true)
            {
                try
                {
                    ...
                    var targetItem = list.FirstOrDefault(x => x.ParameterName == "Tank2OutsideLevel");
                    if (targetItem != null && double.TryParse(targetItem.Value, out double parsedValue))
                    {
                        Tank2OutsideLevel = parsedValue != 0 ? 5 - parsedValue : parsedValue;
                    ...
                    var targetItem1 = list.FirstOrDefault(x => x.ParameterName == "Tank2InsideLevel");
                    if (targetItem1 != null && double.TryParse(targetItem1.Value, out double parsedValue1))
                    {
                        ...
                        {
                            Tank2InsideLevel = parsedValue1;
                        }


                    }

                    var targetItem2 = list.FirstOrDefault(x => x.ParameterName == "Tank2CoolingLevel");
                    if (targetItem2 != null && double.TryParse(targetItem2.Value, out double parsedValue2))
                    {
                        Tank2CoolingLevel = parsedValue2 != 0 ? 5 - parsedValue2 : parsedValue2;
                    }
